...
/* extern */ 
int x3f_get_camf_text(x3f_t *x3f, const char *name, char **text){
...
/* extern */ 
int x3f_get_camf_matrix_var(x3f_t *x3f, const char *name, int *dim0, 
                    int *dim1, int *dim2, matrix_type_t mtype, void **matrix){
    x3f_directory_entry_t *DE = x3f_get_camf(x3f);
    x3f_directory_entry_header_t *DEH;
    x3f_camf_t *CAMF;
    camf_entry_t *table;
    ...
    for (i=0; i<CAMF->entry_table.size; i++){
        ...
        if (!strcmp(name, entry->name_address)){
            ...
            if (entry->matrix_decoded_type != mtype){
            ...
            switch (entry->matrix_dim){
                case 3:
                    if (dim2==nullptr || dim1==nullptr || dim0==nullptr){
...
/* extern */ 
int x3f_get_camf_matrix(x3f_t *x3f, const char *name, int dim0, int dim1, int dim2,
        ...
        if (!strcmp(name, entry->name_address)){
            ...
            switch (entry->matrix_dim){
                case 3:
                    ...
                    break;
                case 2:
                    if (dim2!=0 || dim1!=entry->matrix_dim_entry[1].size ||
                                    dim0!=entry->matrix_dim_entry[0].size){
                        x3f_printf(DEBUG, "CAMF entry - wrong dimension size: %s\n", name);
                        return 0;
                    }
                    break;
                case 1:
                    if (dim2!=0 || dim1!=0 || dim0!=entry->matrix_dim_entry[0].size){
                        x3f_printf(DEBUG, "CAMF entry - wrong dimension size: %s\n", name);
                        return 0;
                    }
                    break;
...
/* extern */ 
int x3f_get_camf_float(x3f_t *x3f, const char *name,  double *val){
  return x3f_get_camf_matrix(x3f, name, 1, 0, 0, M_FLOAT, val);
}


/* extern */ 
int x3f_get_camf_float_vector(x3f_t *x3f, const char *name,  double *val){
    return x3f_get_camf_matrix(x3f, name, 3, 0, 0, M_FLOAT, val);
}


/* extern */ 
int x3f_get_camf_unsigned(x3f_t *x3f, const char *name,  uint32_t *val){
    return x3f_get_camf_matrix(x3f, name, 1, 0, 0, M_UINT, val);
}


/* extern */ 
int x3f_get_camf_signed(x3f_t *x3f, const char *name,  int32_t *val){
    return x3f_get_camf_matrix(x3f, name, 1, 0, 0, M_INT, val);
}


/* extern */ 
int x3f_get_camf_signed_vector(x3f_t *x3f, const char *name, int32_t *val){
    return x3f_get_camf_matrix(x3f, name, 3, 0, 0, M_INT, val);
}


/* extern */ 
int x3f_get_camf_property_list(x3f_t *x3f, const char *list,
...
/* extern */ 
int x3f_get_camf_property(x3f_t *x3f, const char *list,
...
/* extern */ 
int x3f_get_prop_entry(x3f_t *x3f, const char *name, char **value){
...
/* extern */ 
const char *x3f_get_wb(x3f_t *x3f){
    uint32_t wb;
...
/* extern */ 
int x3f_get_camf_matrix_for_wb(x3f_t *x3f, const char *list, const char *wb,
					    int dim0, int dim1, double *matrix){
    char *matrix_name;

    if (!x3f_get_camf_property(x3f, list, wb, &matrix_name)){
        if (!strcmp(wb, "Daylight")) 
            /* Workaround for bug in SD1 */
            return x3f_get_camf_matrix_for_wb(x3f, list, "Sunlight", dim0, dim1, matrix);
        return 0;
    }
...
static int x3f_is_TRUE_engine(x3f_t *x3f){
    ...
    if ((x3f_get_camf_property_list(x3f, "WhiteBalanceColorCorrections",
         ...
         &names, &values, &num)) && 
         (x3f_get_camf_property_list(x3f, "WhiteBalanceGains", &names, &values, &num) ||
         x3f_get_camf_property_list(x3f, "DP1_WhiteBalanceGains", &names, &values, &num)))
        return 1;
    else
        return 0;
}
 ...
 *                                                          */
/* extern */ 
int x3f_get_max_raw(x3f_t *x3f, uint32_t *max_raw){
