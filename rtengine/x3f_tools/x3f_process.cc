...
#define BOTTOM 1
    ...
    /* Workaround for bug in DP2 firmware. DarkShieldBottom 
...
int x3f_get_bmt_to_xyz(x3f_t *x3f, const char *wb, double *bmt_to_xyz){
    ...
    if (x3f_get_camf_matrix_for_wb(x3f, "WhiteBalanceColorCorrections", wb, 3, 3, cc_matrix) ||
        x3f_get_camf_matrix_for_wb(x3f, "DP1_WhiteBalanceColorCorrections", wb, 3, 3, cc_matrix)){
...
int x3f_get_raw_to_xyz(x3f_t *x3f, const char *wb, double *raw_to_xyz){
    ...
    if (!x3f_get_gain(x3f, wb, gain)) 
        return 0;
    if (!x3f_get_bmt_to_xyz(x3f, wb, bmt_to_xyz)) 
        return 0;
...
static void interpolate_bad_pixels(x3f_t *x3f, x3f_area16_t *image, int colors){
     ...
     * collects all bad pixels both in the list 'bad_pixel_list' 
     * and the vector 'bad_pixel_vec' */
    if (colors==3){
        ...
        /* TODO: should those really be interpolated over, 
         * or should they be rescaled instead? */
        if (x3f_get_camf_matrix(x3f, "HighlightPixelsInfo", 2, 2, 0, M_UINT,hpinfo)){
    ...
    if ((colors==1 && x3f_get_camf_matrix_var(x3f, "BadPixelsLumaF23",
        ...
        for (i=0, row=-1; i<bpf23_len; i++)
            if (row==-1) 
                ...
                MARK_PIX(bad_pixel_list, bad_pixel_vec, bpf23[i], row,
    ...
    while (bad_pixel_list){
        ...
        for (p=bad_pixel_list; p && (pn=p->next, 1); p=pn){
            ...
            if (!TEST_PIX(bad_pixel_vec, p->c - 1, p->r, image->columns, image->rows))
                num++, inp[0] = &image->data[p->r * image->row_stride + (p->c - 1) * image->channels];
            if (!TEST_PIX(bad_pixel_vec, p->c + 1, p->r, image->columns, image->rows))
                num++, inp[1] = &image->data[p->r * image->row_stride + (p->c + 1) * image->channels];
            if (!TEST_PIX(bad_pixel_vec, p->c, p->r - 1, image->columns, image->rows))
                num++, inp[2] = &image->data[(p->r - 1) * image->row_stride + p->c * image->channels];
            if (!TEST_PIX(bad_pixel_vec, p->c, p->r + 1, image->columns, image->rows))
                num++, inp[3] = &image->data[(p->r + 1) * image->row_stride + p->c * image->channels];
            ...
            if (inp[0] && inp[1] && inp[2] && inp[3])
                ...
                /* ... nope - it was not possible. 
                 * Look at next without doing interpolation.  */
                stats.left++; 
                continue;
            }
...
static int preprocess_data(x3f_t *x3f, int fix_bad, const char *wb, x3f_image_levels_t *ilevels){
    ...
    if (!x3f_image_area(x3f, &image) || image.channels<3) 
        return 0;
    ...
    for (row=0; row<image.rows; row++)
        for (col=0; col<image.columns; col++)
            for (color=0; color<colors_in; color++){
                ...
                int32_t out = (int32_t)round(scale[color] * (*valp - black_level[color]) +
                              ilevels->black[color]);
    ...
    if (quattro){
        ...
        for (row=0; row<image.rows; row++)
            for (col=0; col<image.columns; col++){
                uint16_t *outp = &image.data[image.row_stride * row + image.channels * col + 2];
                uint16_t *row1 = &qtop.data[qtop.row_stride * 2 * row + qtop.channels * 2 *col];
                uint16_t *row2 = &qtop.data[qtop.row_stride * (2 * row + 1) + qtop.channels * 2 *col];
                uint32_t sum = row1[0] + row1[qtop.channels] + row2[0] + row2[qtop.channels];
                int32_t out = (int32_t)round(scale[2] * (sum / 4.0 - black_level[2]) + ilevels->black[2]);
                if (out<0) 
                    *outp = 0;
                else if (out>65535) 
                    *outp = 65535;
        ...
        for (row=0; row<qtop.rows; row++)
            for (col=0; col<qtop.columns; col++){
                ...
                int32_t out = (int32_t)round(scale[2] * (*valp - black_level[2]) + ilevels->black[2]);
                if (out<0) 
                    *valp = 0;
                else if (out>65535) 
                    *valp = 65535;
    ...
    for (row=0; row<image->rows; row++){
        for (col=0; col<image->columns; col++){
            ...
            for (color=0; color<3; color++){
                ...
                input[color] = x3f_calc_spatial_gain(sgain, sgain_num, row, col, color,
                               image->rows, image->columns) * (*valp[color] - ilevels->black[color]) /
...
static int expand_quattro(x3f_t *x3f, int denoise, x3f_area16_t *expanded){
    ...
    if (denoise && !x3f_crop_area_camf(x3f, "ActiveImageArea", &image, 1, &active)){
        active = image;
        x3f_printf(WARN, "Could not get active area, denoising entire image\n");
    }
    ...
    if (!x3f_crop_area(rect, &qtop, &qtop_crop)) 
        return 0;
    ...
    expanded->buf = malloc(expanded->rows*expanded->row_stride*sizeof(uint16_t));
    expanded->data = static_cast<uint16_t*>(expanded->buf);

    if (denoise && !x3f_crop_area_camf(x3f, "ActiveImageArea", expanded, 0, &active_exp)){
        active_exp = *expanded;
        x3f_printf(WARN, "Could not get active area, denoising entire image\n");
    }
...
int x3f_get_image(x3f_t *x3f, x3f_area16_t *image, x3f_image_levels_t *ilevels,
    ...
    if (!x3f_image_area(x3f, &original_image)) 
        return 0;
    if (!crop || !x3f_crop_area_camf(x3f, "ActiveImageArea", &original_image, 1, image))
        *image = original_image;
...
int x3f_get_preview(x3f_t *x3f, x3f_area16_t *image, x3f_image_levels_t *ilevels,
    ...
    preview->buf = malloc(preview->rows * preview->row_stride * sizeof(uint8_t));
    preview->data = static_cast<uint8_t*>(preview->buf);
    ...
    for (row=0; row<preview->rows; row++){
        for (col=0; col<preview->columns; col++){
            ...
            for (color=0; color<3; color++){
                ...
                input[color] = x3f_calc_spatial_gain(sgain, sgain_num, row, col, color,
                               preview->rows, preview->columns) * ((double)acc / reduction2 
                               - ilevels->black[color]) / (ilevels->white[color] - ilevels->black[color]);
            }
